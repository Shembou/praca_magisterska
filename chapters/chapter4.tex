...
Architektura przygotowanego środowiska testowego została zobrazowana na Rysunku~\ref{fig:file_structure} (str.~\pageref{fig:file_structure}). Struktura projektu obejmuje pięć głównych katalogów, z których każdy jest dedykowany do realizacji odrębnego zakresu badań. Architektura projektowa aplikacji została zaprojektowana w oparciu o koncepcje systemów rozproszonych i organizacji aplikacji opisane w publikacji
Martina Kleppmanna\cite{designing_data_intensive_applications}.
...
Katalog zawiera programy napisane w języku \texttt{Rust}, które są kompilowane do formatu \texttt{WebAssembly} (WASM). Powstałe moduły binarne są następnie wykorzystywane w środowisku przeglądarki internetowej, umożliwiając uruchomienie kodu o wysokiej wydajności po stronie klienta.

latex\subsection{Katalog wasm-showcase}
Katalog \texttt{wasm-showcase} reprezentuje w pełni funkcjonalną aplikację webową o charakterze demonstracyjnym, ilustrującą możliwości oraz zalety technologii \texttt{WebAssembly}. Fundamenty architektoniczne oraz implementowane wzorce programistyczne czerpią z koncepcji zaprezentowanych w publikacji\cite{react_18_design_patterns}. Organizacja projektu opiera się na następujących elementach składowych:
\begin{itemize}
...
Plik konfiguracyjny systemu kontroli wersji \texttt{Git}, zawierający listę plików i katalogów wykluczonych z repozytorium. Typowo obejmuje katalogi tymczasowe (target, node\_modules), pliki konfiguracyjne środowiska oraz artefakty kompilacji, które nie powinny być śledzone w historii zmian projektu.
...
\begin{enumerate}
	\item \textbf{Transmisja małych pakietów danych} -- test polegający na cyklicznym odpytywaniu serwera o dane o rozmiarze \texttt{1~KB}. Pomiar ten pozwala ocenić narzut protokołu oraz czas odpowiedzi dla typowych żądań zawierających niewielką ilość informacji.
	
	\item \textbf{Transmisja dużych pakietów danych} -- test analogiczny do poprzedniego, z tą różnicą, że rozmiar przesyłanych danych wynosi \texttt{1~MB}. Eksperyment ten umożliwia ocenę wydajności protokołów w scenariuszu transferu większych zasobów.
	
	\item \textbf{Head-of-Line Blocking} -- test weryfikujący występowanie zjawiska blokowania głowy kolejki, w którym opóźnienie w przetwarzaniu jednego żądania wpływa na obsługę kolejnych żądań w ramach tego samego połączenia. Testom został poodany tylko protokół \texttt{HTTP/1.1} oraz \texttt{HTTP/2}.
	
	\item \textbf{Strumieniowanie danych} -- test przeprowadzany wyłącznie dla protokołów wspierających tryb strumieniowy (streaming). Ocenie podlega efektywność przesyłania danych w sposób ciągły, bez konieczności oczekiwania na kompletną odpowiedź serwera. Testom został poodany tylko protokół \texttt{HTTP/1.1} oraz \texttt{HTTP/2}.
\end{enumerate}
...
Struktura danych testowych jest generowana w sposób pokazany na listunku ~\ref{lst:user_struct}:
...
\begin{enumerate}
	\item \textbf{Czas serializacji} -- czas potrzebny na przekształcenie struktury danych z reprezentacji obiektowej języka Rust do formatu serializowanego. Pomiar wyrażony w milisekundach (\texttt{ms}) i uśredniony na podstawie wielokrotnych iteracji.
	
	\item \textbf{Czas deserializacji} -- czas wymagany do odtworzenia struktury obiektowej z danych w formacie serializowanym. Pomiar wyrażony w milisekundach (\texttt{ms}) i uśredniony na podstawie wielokrotnych iteracji.
	
	\item \textbf{Rozmiar zserializowanych danych} -- całkowita wielkość danych po procesie serializacji, wyrażona w bajtach (\texttt{B}). Parametr ten pozwala ocenić efektywność kompresji oraz narzut protokołu dla różnych formatów wymiany danych.
\end{enumerate}
...
Katalogi \textit{wasm} oraz \textit{wasm-showcase} obejmują zbiór aplikacji eksperymentalnych napisanych w języku \texttt{Rust}, które zostały skompilowane do formatu \texttt{WebAssembly} (WASM) z wykorzystaniem narzędzia \textit{wasm-pack}. Celem tych aplikacji jest analiza wpływu wykorzystania technologii WebAssembly na wydajność wykonywania obliczeń po stronie klienta w środowisku przeglądarki internetowej.

W ramach przeprowadzonych eksperymentów porównywany jest czas wykonania wybranych operacji matematycznych realizowanych bezpośrednio w języku JavaScript oraz analogicznych operacji wykonywanych przez skompilowane moduły \texttt{WebAssembly}. Takie zestawienie umożliwia ocenę potencjalnych korzyści wynikających z wykorzystania kodu natywnego kompilowanego do WASM w kontekście obliczeń o zwiększonej złożoności.
